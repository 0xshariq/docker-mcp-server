...
if (args.includes('--help') || args.includes('-h')) {
  try {
    const helpFilePath = path.join(__dirname, '..', '..', 'help', 'advanced', 'docker-volume.json');
    ...
    if (helpContent.volume_drivers) {
      console.log('\nVolume Drivers:');
      helpContent.volume_drivers.forEach(driver => {
        console.log(`  ${driver.driver.padEnd(15)} - ${driver.description}`);
      });
    }
