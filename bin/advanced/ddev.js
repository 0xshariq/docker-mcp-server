...
if (args.includes('--help') || args.includes('-h')) {
  try {
    const helpFilePath = path.join(__dirname, '..', '..', 'help', 'workflows', 'docker-dev.json');
    ...
    if (helpContent.development_profiles) {
      console.log('\nDevelopment Profiles:');
      helpContent.development_profiles.forEach(profile => {
        console.log(`  ${profile.profile.padEnd(12)} - ${profile.description}`);
      });
    }
