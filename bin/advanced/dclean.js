...
if (args.includes('--help') || args.includes('-h')) {
  try {
    const helpFilePath = path.join(__dirname, '..', '..', 'help', 'advanced', 'docker-clean.json');
    ...
    console.log('\nCleanup Scopes:');
    helpContent.cleanup_scopes.forEach(scope => {
      console.log(`  ${scope}`);
    });
    ...
    if (helpContent.safety_warnings) {
      console.log('\nSafety Warnings:');
      helpContent.safety_warnings.forEach(warning => {
        console.log(`  ${warning}`);
      });
    }
