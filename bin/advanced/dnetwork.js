...
if (args.includes('--help') || args.includes('-h')) {
  try {
    const helpFilePath = path.join(__dirname, '..', '..', 'help', 'advanced', 'docker-network.json');
    ...
    if (helpContent.network_types) {
      console.log('\nNetwork Types:');
      helpContent.network_types.forEach(type => {
        console.log(`  ${type.type.padEnd(15)} - ${type.description}`);
      });
    }
