...
if (args.includes('--help') || args.includes('-h')) {
  try {
    const helpFilePath = path.join(__dirname, '..', '..', 'help', 'advanced', 'docker-inspect.json');
    ...
    if (helpContent.format_examples) {
      console.log('\nFormat Examples:');
      helpContent.format_examples.forEach(example => {
        console.log(`  ${example.format.padEnd(25)} - ${example.description}`);
      });
    }
